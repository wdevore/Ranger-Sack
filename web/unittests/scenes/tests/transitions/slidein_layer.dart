 ...
 * The background is gradient and includes two vertical bars.
 */
class SlideInLayer extends Ranger.BackgroundLayer {
  ...
  factory SlideInLayer.basic([bool centered = true, int width, int height]) {
    SlideInLayer layer = new SlideInLayer();
  ...
  bool onMouseDown(MouseEvent event) {
    ...
    if (_rightArrow.pointInside(nodeP.v)) {
      _transition(Ranger.TransitionSlideIn.FROM_RIGHT, Ranger.color4IFromHex("#0067a0"), Ranger.color4IFromHex("#b8dde1"));
      return true;
    }
    ...
    if (_leftArrow.pointInside(nodeP.v)) {
      _transition(Ranger.TransitionSlideIn.FROM_LEFT, Ranger.color4IFromHex("#40c1ac"), Ranger.color4IFromHex("#18332f"));
      return true;
    }
    ...
    if (_upArrow.pointInside(nodeP.v)) {
      _transition(Ranger.TransitionSlideIn.FROM_TOP, Ranger.color4IFromHex("#d22630"), Ranger.color4IFromHex("#e04e39"));
      return true;
    }
    ...
    if (_downArrow.pointInside(nodeP.v)) {
      _transition(Ranger.TransitionSlideIn.FROM_BOTTOM, Ranger.color4IFromHex("#4f2c1d"), Ranger.color4IFromHex("#cda788"));
      return true;
    }
  ...
  void _configure() {
    ...
    _widerBar.fillColor = Ranger.color4IFromHex("#ffaa00aa").toString();
    _widerBar.scaleTo(70.0, app.designSize.height);
    _widerBar.setPosition(-hGap + (hGap * 0.15), 0.0);
    _widerBar.center();

    addChild(_narrowBar, 10, 126);
    _narrowBar.fillColor = Ranger.color4IFromHex("#ffaa00aa").toString();
    _narrowBar.scaleTo(25.0, app.designSize.height);
    _narrowBar.setPosition(-hGap + (hGap * 0.30), 0.0);
    _narrowBar.center();
    ...
    _title.text = "Slide-In transitions";
